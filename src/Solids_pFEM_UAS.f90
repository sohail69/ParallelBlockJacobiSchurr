...
!-------------------------------
! Takes a Scalar shape function
! derivative and returns the 
! N-Dimensional vector argument
!-------------------------------
PURE FUNCTION SDF_func(dNi, I, J, m, nDIM, nod) RESULT(dNij)
   INTEGER,  INTENT(IN) :: I, J, m, nDIM, nod
  REAL(iwp), INTENT(IN) :: dNi(ndim,nod)
  REAL(iwp)             :: dNij

  IF(I /= J) dNij = 0._iwp
  IF(I == J) dNij = dNi(I,m)
END FUNCTION SDF_func
